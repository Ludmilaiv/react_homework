...
export const MessageFunction: FC<IMessageProps> = ({messageList, setMessageList}) => {
  ...
  return <form className="app__message message-function" action='' onSubmit={e => e.preventDefault()}>
    ...
    <button className="message-function__button" type="button" onClick={() => {
      setMessageList([{text: textValue, author: AUTHOR.USER, authorName: author, id: nanoid()}, ...messageList]);
      setTextValue('');
      setAuthor('');
    }}>Отправить</button>
  </form>;
